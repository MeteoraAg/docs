...
sidebarTitle: "Becoming a Liquidity Provider"
description: "Learn what liquidity providers do, how AMMs work, how LPs earn fees and incentives, and how Meteora's DLMM, DAMM v1, and DAMM v2 pools differ."
llmsDescription: "User guide for becoming a liquidity provider on Meteora. Explains that liquidity providers deposit token pairs into liquidity pools so traders can swap against them, and that LPs may earn swap fees, bonus incentives, yield farming rewards, or protocol incentives based on their share of liquidity. Introduces key LP risks, including receiving a different token balance than originally deposited due to price changes. Covers Automated Market Maker (AMM) basics, including how decentralized liquidity pools replace centralized orderbooks, how the constant product formula x * y = k works, how traders swap against pool liquidity, and how LP fees are distributed. Compares Meteora's main pool types for LPs: DLMM, which uses concentrated liquidity, discrete price bins, zero-slippage bins, volatility strategies, dynamic fees, single-sided deposits, and active range management; DAMM v1, a simpler full-range constant product Dynamic AMM with fixed fees, yield-layer vault allocation, permissionless pool creation, LP tokens, and liquidity burn options; and DAMM v2, a configurable constant product Dynamic AMM with SPL and Token 2022 support, dynamic fees, Anti-Sniper mechanisms such as Fee Scheduler and Rate Limiter, fee token selection, position NFTs, built-in farming, lower pool creation costs, liquidity locking options, single-sided liquidity, and configurable price ranges."
---
...
<iframe
  ...
  src="https://www.youtube.com/embed/3d0yTfwECwk?si=RSbDM-p15KnBk-nk"
  title="Being a Liquidity Provider"
  frameBorder="0"
