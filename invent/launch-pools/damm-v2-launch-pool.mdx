...
description: "Learn how to create a Meteora DAMM v2 launch pool with balanced or one-sided liquidity, configurable fees, activation controls, optional Alpha Vault, and position management settings."
llmsDescription: "Guide to creating a DAMM v2 launch pool with Meteora Invent Studio, covering setup, local validator, environment variables, keypair generation, damm_v2_config.jsonc, quote mint, createBaseToken, creator, base and quote amounts, initial price, min and max price, base fee modes, rate limiter, market cap scheduler, dynamic fees, collect fee mode, compounding fees, activation controls, add liquidity, split position, optional Alpha Vault, and balanced or one-sided pool commands."
---
