...
sidebarTitle: "DLMM Launch Pool"
description: "Learn how to create a Meteora DLMM launch pool with configurable price bins, dynamic fees, activation controls, optional Alpha Vault, liquidity seeding, and limit orders."
llmsDescription: "Guide to creating a DLMM launch pool with Meteora Invent Studio, covering setup, local validator, environment variables, keypair generation, dlmm_config.jsonc, quote mint, createBaseToken, bin step, fee bps, initial price, activation type and point, creator pool on-off control, concrete function type, collect fee mode, LFG seed liquidity, single-bin seed liquidity, DLMM pool status, optional Alpha Vault, dlmm-create-pool and seed commands, and dlmm-place-limit-order, dlmm-get-limit-orders, and dlmm-cancel-limit-order limit order commands."
---
...
<Steps>
  ...
  <Frame>
   ...
   <Step title="Optional: Start a Local Test Validator">
    ...
    <Frame>
    <img src="/invent/assets/start-test-validator.webp" />
    </Frame>
  ...
  <Frame>
   <img src="/invent/assets/keypair.webp" />
  </Frame>
  ...
  <Frame caption="This example includes creating of baseMint token before creating the DLMM launch pool.">
   <img src="/invent/assets/dlmm-create-pool.webp" />
  </Frame>
    ...
    <Tabs>
      <Tab title="dlmm-seed-liquidity-lfg" icon="chart-bar">
        *If you want to seed your pool with liquidity using the LFG model, you can run the following command.*
        ...
        <Frame>
        <img src="/invent/assets/dlmm-seed-liquidity-lfg.webp" />
        </Frame>
      </Tab>
      <Tab title="dlmm-seed-liquidity-single-bin" icon="chart-area">
        *If you want to seed your pool with liquidity in a single bin, you can run the following command.*
        ...
        <Frame>
          <img src="/invent/assets/dlmm-seed-liquidity-single-bin.webp" />
        </Frame>
      </Tab>
    </Tabs>
  </Step>
  <Step title="Place Limit Orders on your DLMM Pool (Optional)">
    ...
    <Note>To learn how DLMM limit orders work under the hood, head to [DLMM Limit Order](/core-products/dlmm/limit-order).</Note>
    ...
    *To cancel an order and withdraw unfilled deposits, filled proceeds, and earned fees, run the following command.*
    ...
    <Tip>Omit the `--limitOrder` flag and set `cancelLimitOrder.cancelAll` to `true` in `dlmm_config.jsonc` to cancel every open order on the pool in one run.</Tip>
  </Step>
</Steps>
