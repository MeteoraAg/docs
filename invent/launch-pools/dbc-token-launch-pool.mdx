...
description: "Learn how to create a Meteora DBC token launch pool with configurable bonding curves, launch fees, token metadata, DAMM migration, and liquidity distribution."
llmsDescription: "Guide to creating a DBC token launch pool with Meteora Invent Studio, covering setup, local validator, environment variables, keypair generation, dbc_config.jsonc, quote mint, curve build modes, token supply and authority settings, fee scheduler, rate limiter, dynamic fee, pool creation fee, creator trading fee split, migration to DAMM v1 or DAMM v2, migration fees, migrated DAMM v2 pool fees, liquidity distribution, locked vesting, leftover receiver, fee claimer, dbcPool metadata, swaps, pool creator transfer, and dbc-create-pool commands."
---
