...
description: "Learn how DLMM Limit Orders place liquidity at selected price bins to create on-chain buy and sell strategies inside limit-order-enabled DLMM pools."
llmsDescription: "Explains DLMM Limit Orders as on-chain liquidity placed at specific price bins in limit-order-enabled pools, including bid-side and ask-side orders, the 50-bin order limit, absolute and relative bin placement, how orders fill when swaps cross target bins, how limit orders differ from LP positions, fee sharing with market-maker liquidity, DCA use cases, take-profit use cases, and execution considerations."
---
...
Limit orders are available only on DLMM pools whose function mode supports limit orders. In the `lb_clmm` program, a pool supports either liquidity mining or limit-order behavior; it does not run both modes at the same time.
