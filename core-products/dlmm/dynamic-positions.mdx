...
At the program level, this is represented by `PositionV2` and loaded through the dynamic position model in `DLMM/programs/lb_clmm`. `PositionV2` starts with the default 70-bin layout and can allocate additional per-bin data when the position grows. The maximum supported position length is **1,400 bins**.
...
lower bin ID                       upper bin ID
     [ bin ][ bin ][ bin ][ bin ][ bin ]
                 active bin may move through this range
```
