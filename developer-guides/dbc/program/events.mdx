...
sidebarTitle: "Events"
description: "Understand DBC dynamic_bonding_curve events emitted by config, transfer-hook, pool, swap, fee claim, migration, metadata, and operator flows."
llmsDescription: "Explains the DBC program event reference for the public Meteora dynamic_bonding_curve program. Covers Anchor event-CPI delivery, event authority PDA requirements, config and transfer-hook config events, metadata events, EvtInitializePool, EvtInitializePoolWithTransferHook, EvtSwap and EvtSwap2, EvtSwap2WithTransferHook, EvtCurveComplete, EvtCurveCompleteWithTransferHook, fee claim events, migration events, surplus and leftover events, operator events, deprecated EvtCreateConfig compatibility behavior, and indexing considerations for trading terminals and analytics."
---
...
| `EvtCreateTokenBadge` | `create_token_badge` | `token_mint` | Index a newly allowlisted quote mint. |
| `EvtCloseTokenBadge` | `close_token_badge` | `token_mint` | Index token badge closure. |
...
| `EvtClaimProtocolFee` | Removed `claim_protocol_fee` | `pool`, `token_base_amount`, `token_quote_amount` | Historical protocol trading fee claim. New claims use `EvtClaimProtocolFee2`. |
