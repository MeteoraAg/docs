...
description: "Understand DBC dynamic_bonding_curve accounts, including configs, virtual pools, metadata, operators, migration state, constants, and PDA seeds."
llmsDescription: "Explains the DBC program account model for the public Meteora dynamic_bonding_curve program. Covers PoolConfig, VirtualPool, PartnerMetadata, VirtualPoolMetadata, Operator, ClaimFeeOperator, MeteoraDammMigrationMetadata, LockEscrow, config signer accounts, token vaults, constants, enums, PDA seeds, state interpretation, and SDK methods for fetching and deriving accounts."
---
...
| `VirtualPool` | PDA from quote mint, base mint, and config | Active DBC pool for one base mint and config. Stores creator, vaults, reserves, current price, fee accruals, metrics, and migration progress. |
