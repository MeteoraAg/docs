---
title: "DBC Bonding Curve Configs"
sidebarTitle: "Bonding Curve Configs"
description: "Configure DBC pool fees, curve shape, token behavior, vesting, migration, and graduated DAMM pool settings."
llmsDescription: "DBC PoolConfig field guide covering partner/creator roles, tokenomics, poolFees, baseFee, dynamicFee, migrationQuoteThreshold, migrationFee, curve points, DAMM v1/v2 migration, liquidity percentages, LP vesting, poolCreationFee, and buildCurve helpers."
---

<Note>
  Every DBC pool is created from a config key that defines pool behavior. The pool address is derived from `baseMint`, `quoteMint`, and `config`.
</Note>

Configs define pre-graduation pool fees, post-graduation pool fees, collect fee mode, dynamic fee behavior, bonding curve shape, token vesting, migration rules, and fee recipients.

This guide explains each config field and points to SDK helpers that can build valid curve parameters for common launch designs.
...
2. Use the build curve functions in the [DBC TypeScript SDK](/developer-guides/dbc/typescript-sdk/getting-started) to create your DBC curve config key. These build curve functions abstracts the math complexity of the bonding curve while still allowing you to have full control over the curve structure.
...
- If you want your pool to automatically migrate when the `poolState.quoteReserve` is greater than or equal to the `poolConfig.migrationQuoteThreshold`, you would need to set the `migrationQuoteThreshold` to have a minimum value of 750 USD. You can read more about our migration keepers [here](/developer-guides/dbc#migration-keepers).
...
- When configuring the curve segments, you would need to adhere to the [math formulas of the bonding curve](/core-products/dbc/formulas) to ensure that the curve is valid.
...
- You can find the list of the pre-defined fee configs [here](/developer-guides/dbc#damm-fee-config-keys).
...
### [buildCurveWithMarketCap](/developer-guides/dbc/typescript-sdk/sdk-functions#buildcurvewithmarketcap)
...
### [buildCurveWithTwoSegments](/developer-guides/dbc/typescript-sdk/sdk-functions#buildcurvewithtwosegments)
...
### [buildCurveWithLiquidityWeights](/developer-guides/dbc/typescript-sdk/sdk-functions#buildcurvewithliquidityweights)
...
### [buildCurveWithMidPrice](/developer-guides/dbc/typescript-sdk/sdk-functions#buildcurvewithmidprice)
...
### [buildCurveWithCustomSqrtPrices](/developer-guides/dbc/typescript-sdk/sdk-functions#buildcurvewithcustomsqrtprices)
