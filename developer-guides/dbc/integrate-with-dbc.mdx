---
title: "DBC Trading Terminal Integration"
sidebarTitle: "Trading Terminal"
description: "Index DBC pool state, chart swaps from EvtSwap2, calculate progress, fees, liquidity, migration state, and referral behavior."
---

Trading terminals should integrate DBC from program account state and DBC swap events. Avoid deriving price from token transfer logs because DBC fee behavior can make transfer amounts differ from the curve amounts used for price and charts.
...
There are 3 ways you can get the quote reserve:
...
Alternatively, you can use the `getQuoteReserveFromNextSqrtPrice` function in the [DBC TypeScript SDK](https://github.com/MeteoraAg/dynamic-bonding-curve-sdk/blob/main/packages/dynamic-bonding-curve/docs.md#getQuoteReserveFromNextSqrtPrice). It mirrors the Rust helper above.
...
<Note>You can refer to the product explanation for the Fee Scheduler [here](/core-products/dbc/fees/fee-scheduler).</Note>
...
<Note>You can refer to the product explanation for the Rate Limiter [here](/core-products/dbc/fees/rate-limiter).</Note>
...
You can refer to the Dynamic Fee explanation [here](/core-products/dbc/fees/dynamic-fees).
...
- We run 2 DBC migration keepers for mainnet pools. Each keeper has its own quote reserve requirements, and the pool must also satisfy `quoteReserve >= migrationQuoteThreshold`.
- You can check the migration keeper details [here](/developer-guides/dbc#migration-keepers).
