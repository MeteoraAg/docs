...
description: "Understand DLMM lb_clmm accounts, including pools, bin arrays, positions, limit orders, oracles, presets, and PDA seeds."
llmsDescription: "Explains the DLMM program account model from the lb_clmm source. Covers LbPair pools, BinArray and bitmap accounts, PositionV2 and dynamic positions, limit order accounts, oracle accounts, preset parameter accounts, token reserves, PDA seeds, nested structs, dynamic account layouts, ownership fields, and which accounts transactions must include."
---
