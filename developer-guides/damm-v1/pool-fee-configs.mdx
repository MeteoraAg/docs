---
title: "DAMM v1 Pool Fee Configs"
sidebarTitle: "Pool Fee Configs"
description: "Explore DAMM v1 pool fee configs, including config-derived pool addresses, public and private config behavior, activation fields, and published mainnet/devnet config keys."
llmsDescription: "Explains DAMM v1 pool fee configs for config-based launch pools. Covers pool address derivation by pool type, public versus private configs, pool_creator_authority behavior, trade fee bps, protocol fee bps, activation duration, vault config key, activation type, and published mainnet and devnet config key lists."
---

<Note>
DAMM v1 pool addresses are derived differently depending on the pool type. Only some pools require a config key; others use token mints and curve information.
...
Configs define the fee parameters, activation behavior, optional alpha-vault configuration, and optional creator authority for config-based DAMM v1 pools.

If `pool_creator_authority` is `11111111111111111111111111111111`, the config is public and any signer can use it to create a DAMM v1 pool.
...
<Note>The list of config key addresses on devnet can be found [here](/api-reference/damm-v1/pools/pool-configs).</Note>
...
<Note>The list of vault config keys on devnet can be found [here](/api-reference/damm-v1/alpha-vault/alpha-vault-configs).</Note>
...
<CardGroup cols={1}>
  <Card title="Fee Scheduler Configuration API" href="/api-reference/damm-v1/fee-configs/fee-configs" icon="link" iconType="solid">
