...
DAMM v1 custom errors are defined in `PoolError`. Anchor custom errors start at code `6000`.
...
| Symptom | Check |
| --- | --- |
| Pool creation fails with fee errors | Convert bps to numerator with `FEE_DENOMINATOR = 100_000`; config and customizable pools validate different fee ranges. |
| Swap fails before math | Check `pool.enabled`, activation point, source/destination token accounts, protocol fee account, and depeg remaining accounts. |
| Stable pool operation fails | Confirm the curve is stable, token multipliers match mint decimals, and depeg accounts are supplied when required. |
| Launch pool activation fails | Verify `activation_type`, current slot or timestamp, alpha-vault timing buffers, and max activation duration. |
| Partner claim fails | Confirm `pool.partner_info.partner_authority`, partner token accounts, and max claim amounts. |
| Protocol fee withdrawal fails | Confirm the operator PDA, permission bitmap, treasury ATAs, and mint restrictions for zap flows. |
