...
`TokenBadge` is a `160` byte account that records an operator-approved Token 2022 mint. Pool creation and reward initialization require this account when `is_supported_mint` returns false for that mint. A badge cannot be created for a mint that is already permissionlessly supported, and it cannot override the native Token 2022 mint rejection. See [Token 2022 support](/core-products/damm-v2/token-2022-support) for the permissionless allowlist and badge review path.
