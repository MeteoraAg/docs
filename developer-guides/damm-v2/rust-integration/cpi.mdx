...
| Plan Token 2022 mint validation | Transfer Hook is permissionless only when both the hook program ID and authority are unset. Other Token 2022 extensions need a token badge for pool creation or reward init. Do not forward transfer-hook account sets as a DLMM-style remaining-account slice; DAMM v2 does not consume them. |
