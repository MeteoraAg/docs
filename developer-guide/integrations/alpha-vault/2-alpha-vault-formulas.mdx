...
**Where:**
- `max_buying_cap` = maximum tokens that can be purchased (pro-rata only)
...
### Individual Quota Calculation

**Remaining Quota:**
```math
\text{remaining\_quota} = \text{individual\_cap} - \text{current\_deposit}
```

**Where:**
- `individual_cap` = maximum deposit allowed per user
- `current_deposit` = user's current deposit amount
