...
<Note> Currently, only SOL or USDC is accepted as the quote token when initializing a Dynamic AMM or DLMM Pool with Alpha Vault in a permissionless setup. Since the Alpha Vault can't tell what the quote token would be, the quote token is limited to SOL or USDC. </Note>
...
```math
\text{depositing\_slot} \rightarrow \text{last\_join\_slot} \rightarrow \text{pre\_activation\_slot} \rightarrow \text{activation\_slot} \rightarrow \text{start\_vesting\_slot} \rightarrow \text{end\_vesting\_slot}
```

**Current Slot Condition:**
```math
\text{current\_slot} < \text{pre\_activation\_slot}
```

<Note> Alpha Vault will start to buy from the pool from `pre_activation_slot` </Note>

### Slot Constraints

```math
\text{depositing\_slot} + 3000 \leq \text{last\_join\_slot}
```

```math
\text{last\_join\_slot} > \text{current\_slot}
```

```math
\text{last\_join\_slot} + 750 = \text{pre\_activation\_slot}
```

```math
\text{pre\_activation\_slot} + 9000 = \text{activation\_slot}
```

```math
\text{activation\_slot} < \text{start\_vesting\_slot}
```

```math
\text{end\_vesting\_slot} \geq \text{start\_vesting\_slot}
```
...
```math
\{\text{depositing\_slot}, \text{activation\_slot}, \text{start\_vesting\_slot}, \text{end\_vesting\_slot}\}
```
...
2. **Calculate Activation Slot:**
   ```math
   \text{activation\_slot} = \text{current\_slot} + 9000 + 750 + \text{buffer}
   ```

3. **Calculate Depositing Slot:**
   ```math
   \text{depositing\_slot} = \text{activation\_slot} - 9000 - 3000 - 750 - \text{buffer}
   ```

4. **Set Vesting Slots:**
   ```math
   \text{end\_vesting\_slot} \geq \text{start\_vesting\_slot} > \text{activation\_slot}
   ```
