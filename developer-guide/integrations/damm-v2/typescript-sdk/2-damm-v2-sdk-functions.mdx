...
**Function**
```typescript
async getQuoteExactOut(params: GetQuoteExactOutParams): Promise<QuoteExactOutResult>
```
...
interface GetQuoteExactOutParams {
  ...
  tokenADecimal: number; // Token decimal number Eg. 9 for SOL
  tokenBDecimal: number; // Token decimal number Eg. 6 for USDC
}
```

**Returns**
An object containing:
- `inputAmount`: The required input amount
- `maxInputAmount`: The maximum input amount with slippage tolerance
- `swapResult`: Object containing the swap calculation results
- `priceImpact`: The price impact of the swap as a percentage
...
const CONFIG = {
    ...
    poolAddress: new PublicKey("8Pm2kZpnxD3hoMmt4bjStX2Pw2Z9abpbHzZxMPqxPmie"),
  };
...
const quote = await cpAmm.getQuoteExactOut({
  outAmount: new BN(100_000_000),
  outputTokenMint: new PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
  slippage: 0.5, // 0.5% slippage
  poolState,
  currentTime: blockTime,
  currentSlot,
...
**Returns**
A transaction builder (`TxBuilder`) that can be used to build, sign, and send the transaction to split one position into multiple positions.
...
**Parameters**
```typescript
interface ClaimPositionFeeParams2 {
  owner: PublicKey;               // The owner of the position
  pool: PublicKey;                // The pool address
  position: PublicKey;            // The position address
  positionNftAccount: PublicKey;  // The position NFT account
