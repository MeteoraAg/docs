...
To calculate the next sqrt price:

```math
\sqrt{P'} = \sqrt{P} \times \frac{L}{L + \Delta x \times \sqrt{P}}
```

To calculate the output amount:

```math
\Delta b = L \times (\sqrt{P} - \sqrt{P'})
```
...
```math
\sqrt{P'} = \sqrt{P} + \frac{\Delta y}{L}
```

To calculate the output amount:

```math
\Delta a = L \times \left(\frac{1}{\sqrt{P'}} - \frac{1}{\sqrt{P}}\right)
```

Where:
- `L` = Total liquidity in the pool
- `√P` = Current square root price
- `√P'` = New square root price after swap
- `Δx` = Input amount of token A
- `Δy` = Input amount of token B
- `Δa` = Output amount of token A
- `Δb` = Output amount of token B
