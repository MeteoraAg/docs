...
// Example using Fee Time Scheduler (Exponential)
...
Fetches and decodes the pool fees from Pod Aligned format.

**Function**

```typescript
async fetchPoolFees(pool: PublicKey): Promise<DecodedPoolFees>
```
...
`DecodedPoolFees` - The decoded pool fees which can be one of:
- `PodAlignedFeeTimeScheduler` - For modes 0 (Linear) and 1 (Exponential)
- `PodAlignedFeeRateLimiter` - For mode 2 (Rate Limiter)
- `PodAlignedFeeMarketCapScheduler` - For modes 3 (Linear) and 4 (Exponential)

**Example**

```typescript
const poolFees = await cpAmm.fetchPoolFees(poolAddress);

// Check the base fee mode to determine the structure
if (poolFees.baseFeeMode === BaseFeeMode.FeeTimeSchedulerLinear ||
    poolFees.baseFeeMode === BaseFeeMode.FeeTimeSchedulerExponential) {
  // Fee Time Scheduler structure
  console.log(`Number of Periods: ${poolFees.numberOfPeriod}`);
  console.log(`Period Frequency: ${poolFees.periodFrequency.toString()}`);
  console.log(`Reduction Factor: ${poolFees.reductionFactor.toString()}`);
} else if (poolFees.baseFeeMode === BaseFeeMode.RateLimiter) {
  // Rate Limiter structure
  console.log(`Fee Increment Bps: ${poolFees.feeIncrementBps}`);
  console.log(`Max Limiter Duration: ${poolFees.maxLimiterDuration}`);
  console.log(`Reference Amount: ${poolFees.referenceAmount.toString()}`);
} else if (poolFees.baseFeeMode === BaseFeeMode.FeeMarketCapSchedulerLinear ||
           poolFees.baseFeeMode === BaseFeeMode.FeeMarketCapSchedulerExponential) {
  // Fee Market Cap Scheduler structure
  console.log(`Number of Periods: ${poolFees.numberOfPeriod}`);
  console.log(`Sqrt Price Step Bps: ${poolFees.sqrtPriceStepBps}`);
  console.log(`Scheduler Expiration Duration: ${poolFees.schedulerExpirationDuration}`);
}
```
...
- Decodes the fees from the on-chain Pod Aligned format to a usable structure
- The returned structure varies based on `baseFeeMode`
...
A `BaseFee` object containing encoded fee parameters.

**Example - Fee Time Scheduler (Linear)**
...
const baseFeeParams = getBaseFeeParams(
  {
    ...
    feeTimeSchedulerParam: {
      ...
      numberOfPeriod: 60,   // 60 decay periods
...
**Example - Fee Market Cap Scheduler (Exponential)**
...
const baseFeeParams = getBaseFeeParams(
  {
    ...
    feeMarketCapSchedulerParam: {
      ...
      numberOfPeriod: 100,                // 100 decay periods
...
const baseFeeParams = getBaseFeeParams(
  {
    ...
    rateLimiterParam: {
      ...
      feeIncrementBps: 10,    // 0.1% increment per reference amount
      referenceAmount: 1,     // 1 token B reference amount
      maxLimiterDuration: 10, // Max 10 slots/seconds for fee increase
      maxFeeBps: 9900,        // 99% max fee cap
    },
  },
  9, // token B decimal
  ActivationType.Slot
...
- Supports 5 base fee modes:
  - `FeeTimeSchedulerLinear` (0): Time-based linear fee reduction
  - `FeeTimeSchedulerExponential` (1): Time-based exponential fee reduction
  - `RateLimiter` (2): Volume-based dynamic fee
  - `FeeMarketCapSchedulerLinear` (3): Price/market cap-based linear fee reduction
  - `FeeMarketCapSchedulerExponential` (4): Price/market cap-based exponential fee reduction
- Calculates period frequency and reduction factors based on the selected mode

**Notes**

- Use `feeTimeSchedulerParam` for modes 0 and 1 (time-based schedulers)
- Use `feeMarketCapSchedulerParam` for modes 3 and 4 (price/market cap-based schedulers)
  - `sqrtPriceStepBps`: Sqrt price increase in bps required to advance one fee reduction period
  - `schedulerExpirationDuration`: Duration after which scheduler expires and uses minimum fee
- Use `rateLimiterParam` for mode 2 (rate limiter)
- Pool Version V1 supports up to 99% max fee (increased from V0's 50% max fee)

---

### getRateLimiterParams

Prepares rate limiter parameters for fee configuration.
...
- `baseFeeBps`: Base fee in basis points (e.g., 100 = 1%)
- `feeIncrementBps`: Fee increment per reference amount in basis points
- `referenceAmount`: Reference trade amount for fee calculation
- `maxLimiterDuration`: Maximum duration (in slots or seconds) for fee accumulation
- `maxFeeBps`: Maximum fee cap in basis points
- `tokenBDecimal`: Decimal places of token B
- `activationType`: The activation type (0: Slot, 1: Timestamp)

**Returns**

A `BaseFee` object with the formatted rate limiter parameters.

**Example**

```typescript
const rateLimiterParams = getRateLimiterParams(
  100,    // 1% base fee
  10,     // 0.1% increment
  1,      // 1 SOL reference amount
...
Prepares fee time scheduler parameters for fee configuration.

**Function**

```typescript
function getFeeTimeSchedulerParams(
...
- `startingBaseFeeBps`: Starting fee in basis points
- `endingBaseFeeBps`: Ending fee in basis points
- `baseFeeMode`: Fee scheduler mode (0: Linear, 1: Exponential)
- `numberOfPeriod`: Number of fee reduction periods
- `totalDuration`: Total duration for fee reduction

**Returns**

A `BaseFee` object with period frequency and reduction factor calculated.

---

### getFeeMarketCapSchedulerParams

Prepares fee market cap scheduler parameters for fee configuration.

**Function**

```typescript
function getFeeMarketCapSchedulerParams(
  startingBaseFeeBps: number,
  endingBaseFeeBps: number,
  baseFeeMode: BaseFeeMode,
  numberOfPeriod: number,
...
- `startingBaseFeeBps`: Starting fee in basis points
- `endingBaseFeeBps`: Ending fee in basis points
- `baseFeeMode`: Fee scheduler mode (3: Linear, 4: Exponential)
- `numberOfPeriod`: Number of fee reduction periods
- `sqrtPriceStepBps`: Sqrt price step in basis points to advance one period
- `schedulerExpirationDuration`: Duration after which scheduler expires

**Returns**

A `BaseFee` object with the calculated reduction factor.
...
## Fee Encode/Decode Functions

These functions encode and decode fee parameters between Borsh format and Pod Aligned formats used on-chain.

### encodeFeeTimeSchedulerParams / decodeFeeTimeSchedulerParams

Encodes and decodes Fee Time Scheduler parameters.

**Functions**

```typescript
function encodeFeeTimeSchedulerParams(
  cliffFeeNumerator: BN,
  numberOfPeriod: number,
  periodFrequency: BN,
  reductionFactor: BN,
...
function decodeFeeTimeSchedulerParams(data: Buffer): BorshFeeTimeScheduler;
```
...
const encoded = encodeFeeTimeSchedulerParams(
  new BN(500000000),   // 50% cliff fee
  60,                  // 60 periods
  new BN(10),          // 10 seconds per period
  new BN(8166666),     // reduction factor
  BaseFeeMode.FeeTimeSchedulerLinear
);

// Decode
const decoded = decodeFeeTimeSchedulerParams(encoded);
// Returns: { cliffFeeNumerator, numberOfPeriod, periodFrequency, reductionFactor, baseFeeMode }
```

---

### encodeFeeMarketCapSchedulerParams / decodeFeeMarketCapSchedulerParams

Encodes and decodes Fee Market Cap Scheduler parameters.

**Functions**

```typescript
function encodeFeeMarketCapSchedulerParams(
  cliffFeeNumerator: BN,
  numberOfPeriod: number,
  sqrtPriceStepBps: number,
  schedulerExpirationDuration: number,
...
function decodeFeeMarketCapSchedulerParams(data: Buffer): BorshFeeMarketCapScheduler;
```
...
// Decode
const decoded = decodeFeeMarketCapSchedulerParams(encoded);
// Returns: { cliffFeeNumerator, numberOfPeriod, sqrtPriceStepBps, schedulerExpirationDuration, reductionFactor, baseFeeMode }
```

---

### encodeFeeRateLimiterParams / decodeFeeRateLimiterParams

Encodes and decodes Fee Rate Limiter parameters.

**Functions**

```typescript
function encodeFeeRateLimiterParams(
  cliffFeeNumerator: BN,
  feeIncrementBps: number,
  maxLimiterDuration: number,
  maxFeeBps: number,
...
function decodeFeeRateLimiterParams(data: Buffer): BorshFeeRateLimiter;
```
...
const encoded = encodeFeeRateLimiterParams(
  new BN(10000000),     // 1% cliff fee
  10,                   // 0.1% fee increment
  10,                   // 10 slots/seconds max duration
  9900,                 // 99% max fee
  new BN(1000000000)    // 1 token B reference amount
);

// Decode
const decoded = decodeFeeRateLimiterParams(encoded);
// Returns: { cliffFeeNumerator, feeIncrementBps, maxLimiterDuration, maxFeeBps, referenceAmount, baseFeeMode }
```

---

### decodePodAlignedFeeTimeScheduler / decodePodAlignedFeeMarketCapScheduler / decodePodAlignedFeeRateLimiter

Decodes fee parameters from the on-chain Pod Aligned format.

**Functions**

```typescript
function decodePodAlignedFeeTimeScheduler(data: Buffer): PodAlignedFeeTimeScheduler;
function decodePodAlignedFeeMarketCapScheduler(data: Buffer): PodAlignedFeeMarketCapScheduler;
function decodePodAlignedFeeRateLimiter(data: Buffer): PodAlignedFeeRateLimiter;
```

**Notes**

- Use these functions when reading fee data directly from on-chain accounts
- The `fetchPoolFees` function uses these internally to decode pool fee data
