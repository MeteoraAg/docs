...
<Note>Every DAMM v2 pool must be created using a config key that defines the pool's parameters. Each DAMM v2 pool is a PDA of `tokenAMint` + `tokenBMint` + `config`.</Note>
...
<Note>Integrators cannot change the configuration for that particular public config key.</Note>
