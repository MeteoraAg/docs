...
- LP vesting is **NOT available** for DAMM v1 (uses fungible LP tokens)

**For DAMM v2 Migration:**
- Can use **permanent locked liquidity only**: `partnerPermanentLockedLiquidityPercentage` + `creatorPermanentLockedLiquidityPercentage` >= 10
- **OR** combination of permanent + vesting liquidity: If using LP vesting (`partnerLiquidityVestingInfoParams` or `creatorLiquidityVestingInfoParams`), the combined locked liquidity at day 1 must be >= 10%
...
- The `partnerLiquidityVestingInfoParams` and `creatorLiquidityVestingInfoParams` are optional parameters that configure LP vesting for the partner and creator positions after migration to DAMM v2.
- These parameters are only applicable when `migrationOption` is set to `DAMM v2`.
- Each vesting info contains the following fields:
    ...
    - `bpsPerPeriod` - Basis points released per period (determines release rate)
...
partnerLiquidityVestingInfoParams: {
    ...
    cliffDurationFromMigrationTime: 7 * 24 * 60 * 60, // 7 days cliff
    totalDuration: 30 * 24 * 60 * 60,                 // 30 days total
    bpsPerPeriod: 100,                                // 1% per period
    numberOfPeriods: 100,                             // 100 periods
}
```
