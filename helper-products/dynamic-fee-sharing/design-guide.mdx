---
title: "Dynamic Fee Sharing Design Guide"
...
Because a fee vault supports a single token mint and a single token vault, the source pool's `CollectFeeMode` must collect fees only in the fee vault's token mint: `OnlyB` for DAMM v2 and `QuoteToken` for DBC. This quote-token-only mode is the default configuration, and partners must always use it.
...
<AccordionGroup>
  ...
  <Accordion title="Pairing a fee vault with a pool that collects fees in both tokens">
