...
Some requests intentionally move from ACT to BUILD because no studio command exists. Examples include DLMM position rebalancing, DBC post-migration withdrawals, custom DAMM v2 position operations, and Presale buyer lifecycle flows.

The skill does not route aggregated swaps across exchanges, create pools on other Solana protocols, or perform unrelated wallet operations. Use the appropriate integration for Jupiter, Raydium, Orca, pump.fun, or general Solana account management.

## How the Skill Stays Current

The repository runs a skill validation script in CI. It checks:

- `SKILL.md` frontmatter and the Agent Skills size guideline
- Internal file references mentioned by the skill and its reference packs
- SDK version pins against `studio/package.json`
- The bundled JSONC templates against the live files under `studio/config`

The protocol packs also identify the SDK source and version they were checked against. If the installed code and the skill disagree, the installed SDK source is the final reference; the agent should call out the mismatch rather than guess.
