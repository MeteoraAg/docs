...
llmsDescription: "Overview of agentic usage with Meteora. Agents should use the Documentation MCP for interactive search and page retrieval, llms.txt and llms-full.txt for static ingestion, raw Markdown exports for exact page context, Developer Guides and API references for implementation details, the Meteora Agent Skill (skills/meteora in the meteora-invent repository) for guided end-to-end onchain workflows, and Meteora Invent or SDK scripts for explicit onchain actions. Meteora's documentation MCP is read-only and should not be treated as a transaction, wallet, or docs-editing MCP server."
---
...
4. **Execute deliberately** through scripts, wallets, SDK transactions, or [Meteora Invent](/invent/actions), with signing controlled by the user. For agents that support skills, install the [Meteora Agent Skill](/agents/skill) — it packages the full action playbook with built-in safety gates.
...
<CardGroup cols={2}>
  ...
  <Card title="Give your agent the full playbook" icon="wand-magic-sparkles" href="/agents/skill">
