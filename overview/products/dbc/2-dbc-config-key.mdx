...
A DBC Config Key is a unique configuration key that you can use to create a Dynamic Bonding Curve token pool. This config key determines the curve behaviour of the DBC token pool, as well as the fees after the token is bonded.
...
You can create a DBC Config Key by using the DBC SDK or via the [launch.meteora.ag](https://launch.meteora.ag)