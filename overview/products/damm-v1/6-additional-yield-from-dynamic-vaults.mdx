...
[Meteora’s Dynamic Vaults](/overview/products/dynamic-vault/1-what-is-dynamic-vault.mdx) operate as a lending aggregator that allocates capital across multiple lending protocols on Solana. We utilize an off-chain keeper called [Hermes](/overview/products/dynamic-vault/2-hermes.mdx) which constantly monitors all lending pools every minute and rebalances allocations to optimize yields and reduce risk.

While there is risk in lending, our Dynamic Vaults [mitigate that risk](/overview/products/dynamic-vault/3-risk-management.mdx) by constantly monitoring lending pool utilization rates and automatically withdrawing funds whenever risk thresholds are hit. Additionally, only a maximum of 30% is ever allocated to any single protocol.
