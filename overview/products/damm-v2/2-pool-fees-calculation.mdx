...
The base fee of a pool can be configured by the pool creator to **either** use:

- **Constant Base Fee**: A fixed fee that is applied to the swap amount.
- **Fee Scheduler**: A fee decay mechanism that decays the fee from starting fee to an ending base fee over a period of time.
