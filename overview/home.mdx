...
**Our mission at Meteora is to build the most secure, sustainable and composable liquidity layer for all of Solana and DeFi.**
...
1. [DLMM Pools](https://docs.meteora.ag/dlmm/dlmm-overview): Dynamic Liquidity Market Maker (DLMM) gives LPs access to dynamic fees to capitalize on volatility, and precise liquidity concentration all in real-time, with the flexibility to select their preferred volatility strategy.

2. [DLMM Launch Pools](https://docs.meteora.ag/meteora-launch-pool/meteora-launch-pool-overview): Specially-designed DLMM for new token launches. It comes fundamentally equipped with a feature set that makes it optimal for bootstrapping liquidity for new tokens and making the tokens accessible on Jupiter and other trading integrations.

3. [DAMM v1 Pools](https://docs.meteora.ag/dynamic-pool/dynamic-amm-pools): Dynamic AMM v1 (DAMM v1) is a constant-product AMM program that supports token prices from 0 to infinity. DAMM v1 is integrated with Dynamic Vaults, where LPs can earn additional yield through utilizing lending sources alongside traditional swap fees, enhancing their returns.

4. [DAMM v2 Pools](https://docs.meteora.ag/damm/damm-v2-overview): Dynamic AMM v2 (DAMM v2) is a constant-product AMM program, with features that optimize transaction fees and provide greater flexibility for liquidity providers, launchpads, and token launches. DAMM v2 comes with SPL and Token 2022 token support, optional concentrated liquidity, position NFT, dynamic fee, on-chain fee scheduler, new fee claiming mechanism and fee token selection, more flexible liquidity locks, and an in-built farming mechanism. Unlike DAMM v1, DAMM v2 is not integrated with Dynamic Vaults. DAMM v2 is a new program, and not an upgrade of the Dynamic AMM v1 program.

5. [Memecoin Pools](https://docs.meteora.ag/damm/memecoin-pools): A subset of Dynamic AMM Pools designed to support memecoin launches with a configurable fee scheduler. Liquidity you add during pool creation will be permanently locked forever to help raise community confidence, but you can still earn fees on your locked liquidity.

6. [Stake2Earn Pools](https://docs.meteora.ag/damm/stake2earn-pools): Memecoin Pools with a stake-to-earn mechanism that allows top token stakers to earn swap fees from locked liquidity.

7. [Dynamic Vaults](https://docs.meteora.ag/damm/dynamic-vaults): Dynamic Vaults optimize capital utilization by dynamically distributing assets to lending pools, generating yields for liquidity providers (LPs) on Dynamic AMM and Memecoin Pools.
